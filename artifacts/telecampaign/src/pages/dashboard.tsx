...
export default function Dashboard() {
  ...
  const [showQuickSendWizard, setShowQuickSendWizard] = useState(false);
  ...
  return (
    <AppLayout activePage="dashboard" title={t("Dashboard")}>
      ...
      {showQuickSend && (
        <section className="mb-8 overflow-hidden rounded-3xl bg-[linear-gradient(115deg,#172b85_0%,#1d4ed8_58%,#2563eb_100%)] text-white shadow-[0_12px_30px_rgba(29,78,216,0.18)]" data-testid="quick-send-entry">
          <div className="flex flex-col gap-6 px-6 py-6 sm:flex-row sm:items-center sm:justify-between sm:px-8 sm:py-7">
            ...
            <button type="button" onClick={() => setShowQuickSendWizard(true)} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-white px-5 py-3 text-[13px] font-extrabold text-[#1a2b88] shadow-sm transition hover:bg-blue-50 active:scale-[0.98]" data-testid="quick-send-open">
      ...
      {showQuickSendWizard && (
        <QuickSendWizard
          onClose={() => setShowQuickSendWizard(false)}
          onCreated={async () => {

...
router.get("/admin/purchase-orders/settings", async (_req, res): Promise<void> => {
  res.json(await getOrderSettings());
});
router.patch("/admin/purchase-orders/settings", async (req, res): Promise<void> => {
  ...
  if (typeof value.vietQrTemplate === "string" && value.vietQrTemplate.length <= 512 && value.vietQrTemplate) {
    try {
      const url = new URL(value.vietQrTemplate);
      validVietQr = url.protocol === "https:" && url.hostname === "img.vietqr.io"
...
router.get("/admin/purchase-orders", async (_req, res): Promise<void> => {
